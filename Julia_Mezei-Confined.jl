...
function Mezei(ChemPot::Float64, h::Float64, L::Float64, T::Float64, R_Cut::Float64 = 3.)
    ...
    σ_w = 0.5;
    λ_w = 1.5;
    #Extra_Particles_Per_Side = 2;
    #N_Slates = floor((λ_w - σ_w) / σ_w);
    #A = LinRange(-L / 2 - Extra_Particles_Per_Side*(2σ_w), L / 2 + Extra_Particles_Per_Side*(2σ_w), )
    rx, ry = -L / 2, -L / 2;
    Slate_x, Slate_y = Float64[], Float64[]
    while rx <= L / 2
        while ry <= L / 2
            append!(Slate_x, rx)
            append!(Slate_y, ry)
    ...
    @inbounds for i = 1:MC_Steps
        ...
        if RN == 2
            ...
            if length(x_Insertion) > 0
                Energy, N_Insertion_Accepted, N_Insertion_Rejected = Insertion_Mezei(h, Beta, ChemPot, L, V, R_Cut, Energy, N_Insertion_Accepted, N_Insertion_Rejected, x, y, z, x_Insertion, y_Insertion, z_Insertion, Pc)
            else
    ...
    Povray_Pov(h, L, ChemPot, T, Slate_x, Slate_y)
    run(`povray $Output_Route/Positions/MC_Animation.ini`)
end

function Movement(h::Float64, L::Float64, Beta::Float64, Displacement::Float64, Energy::Float64, N_Movement_Accepted::Int64, N_Movement_Rejected::Int64, N_Displacement_Accepted::Int64, R_Cut::Float64, x::Array{Float64, 1}, y::Array{Float64, 1}, z::Array{Float64, 1})
    j = rand(1:length(x))
...
function Insertion(h::Float64, L::Float64, V::Float64, ChemPot::Float64, Beta::Float64, R_Cut::Float64, x::Array{Float64, 1}, y::Array{Float64, 1}, z::Array{Float64, 1}, Energy::Float64, N_Insertion_Accepted::Int64, N_Insertion_Rejected::Int64)
...
function Removal(h::Float64, L::Float64, V::Float64, Beta::Float64, ChemPot::Float64, R_Cut::Float64, Energy::Float64, N_Removal_Accepted::Int64, N_Removal_Rejected::Int64, x::Array{Float64, 1}, y::Array{Float64, 1}, z::Array{Float64, 1})
    j = rand(1:length(x));
...
function Removal_Mezei(Pc_Interpolation::Float64, h::Float64, L::Float64, V::Float64, Beta::Float64, ChemPot::Float64, R_Cut::Float64, Energy::Float64, N_Removal_Accepted::Int64, N_Removal_Rejected::Int64, x::Array{Float64, 1}, y::Array{Float64, 1}, z::Array{Float64, 1})
    j = rand(1:length(x))
    Energy_Removal = Energy_Calculation(h, L, R_Cut, x[j], y[j], z[j], x, y, z)
...
function u_ParticleParticle(r2::Float64)
    if r2 <= 1
        return Inf
    elseif r2 <= 1.5^2
        return -1
...
function u_ParticleWall(h::Float64, z::Float64)
    Delta_z = h / 2 - abs(z);
    if Delta_z <= 0.5
        return Inf
    elseif Delta_z <= 1.5
...
function Energy_Calculation(h::Float64, L::Float64, R_Cut::Float64, rx::Float64, ry::Float64, rz::Float64, x::Array{Float64, 1}, y::Array{Float64, 1}, z::Array{Float64, 1})
...
function Povray_Pov(h::Float64, L::Float64, ChemPot::Float64, T::Float64, Slate_x::Array{Float64, 1}, Slate_y::Array{Float64, 1})
    ...
    for i = 1:length(Slate_x)
        println(Pov_File, "Slate($(Slate_x[i]), $(Slate_y[i]), h / 2 + 0.5, 0.5)")
        println(Pov_File, "Slate($(Slate_x[i]), $(Slate_y[i]), -h / 2 - 0.5, 0.5)")
    end
    close(Pov_File)
end
