...
function Mezei(ChemPot::Float64, h::Float64, L::Float64, T::Float64, σ_p::Float64, λ_p::Float64, σ_w::Float64, λ_w::Float64, Number_Run::Int64, Total_Run::Int64, Patch_Percentage::Int64, R_Cut::Float64 = 3.)
    ...
    @inbounds for i = 1:MC_Steps
        ...
        if i > MC_Relaxation_Steps && i % .01MC_Equilibrium_Steps == 0
            println("$(convert(Int64, floor(100(i - MC_Relaxation_Steps) / MC_Equilibrium_Steps)))% Equilibrium ($N_Measurements Measurements). [$Number_Run / $Total_Run]")
            println("U / N = $(round(Energy / length(x), digits = 6))")
            println("N = $(length(x))")
            println("Density = $(round(length(x) / V, digits = 6))")
            println("Max Displacement = $(round(Displacement, digits = 6))")
        ...
        if i % MC_Measurement == 0
            ...
            z_Displacement > h / 8. ? z_Displacement = h / 8. : nothing
    ...
    println(g_x_File, "#x\t#Density\tsigma\n")
    @inbounds for i = 1:N_Bins
        r[i] = round( - L / 2 + (i - 0.5) * Delta, digits = 6)
        println(g_x_File, "$(r[i])\t$(round(g_x_Mean[i], digits = 6))\t$(round(g_x_Std[i], digits = 6))")
    end
    close(g_x_File)
    Distribution_x_Plot = plot(r, g_x_Mean, ribbon = g_x_Std, fillalpha = 0.2, legend = false, xlabel = "x", ylabel = "Density", width = 3, size = [1200, 800], ylims = (0, g_x_Max))
    ...
    println(g_y_File, "#y\t#Density\tsigma\n")
    @inbounds for i = 1:N_Bins
        r[i] = round( - L / 2 + (i - 0.5) * Delta, digits = 6)
    ...
    println(g_z_File, "#z\t#Density\tsigma\n")
    @inbounds for i = 1:N_Bins
...
function Potential(N_Bins::Int64, L::Float64, h::Float64, σ_w::Float64, λ_w::Float64, x::Array{Float64, 1}, y::Array{Float64, 1}, z::Array{Float64, 1})
    Delta = h / N_Bins;
    PotentialFunction = zeros(Float64, N_Bins);
    N_Slab = 2 * convert(Int64, ceil(L / (2σ_w) + 3 + 1) * ceil(L / (√3 * 2σ_w) + √3))
    for i = 1:length(x)
...
function Cycled_Mezei()
    ...
    y = ones(3)
    title = Plots.scatter(y, marker = 0, markeralpha = 0, annotations=(2, y[2], Plots.text("T = $T, Chemical Potential = $ChemPot", :black, 25)), axis=false, grid = false, leg=false,size=(1920,50))
    
    x_Plot = plot(p_x[1], p_x[2], p_x[3], p_x[4], p_x[5], p_x[6], p_x[7], p_x[8], p_x[9], layout = (3, 3), size = [1920, 1080])
    Complete_x_Plot = plot(title, x_Plot, layout = grid(2, 1, heights = [0.025, 0.975]))
    savefig(Complete_x_Plot, "$Output_Route/Distribution_x_$T")
    
    y_Plot = plot(p_y[1], p_y[2], p_y[3], p_y[4], p_y[5], p_y[6], p_y[7], p_y[8], p_y[9], layout = (3, 3), size = [1920, 1080])
    Complete_y_Plot = plot(title, y_Plot, layout = grid(2, 1, heights = [0.025, 0.975]))
    savefig(Complete_y_Plot, "$Output_Route/Distribution_y_$T")

    z_Plot = plot(p_z[1], p_z[2], p_z[3], p_z[4], p_z[5], p_z[6], p_z[7], p_z[8], p_z[9], layout = (3, 3), size = [1920, 1080])
    Complete_z_Plot = plot(title, z_Plot, layout = grid(2, 1, heights = [0.025, 0.975]))
    savefig(Complete_z_Plot, "$Output_Route/Distribution_z_$T")

    Energy_Plot = plot(p_Energy[1], p_Energy[2], p_Energy[3], p_Energy[4], p_Energy[5], p_Energy[6], p_Energy[7], p_Energy[8], p_Energy[9], layout = (3, 3), size = [1920, 1080])
    ...
    Energy_Histogram_Plot = plot(p_Histogram_Energy[1], p_Histogram_Energy[2], p_Histogram_Energy[3], p_Histogram_Energy[4], p_Histogram_Energy[5], p_Histogram_Energy[6], p_Histogram_Energy[7], p_Histogram_Energy[8], p_Histogram_Energy[9], layout = (3, 3), size = [1920, 1080])
    ...
    Energy_Average_Plot = plot(p_Average_Energy[1], p_Average_Energy[2], p_Average_Energy[3], p_Average_Energy[4], p_Average_Energy[5], p_Average_Energy[6], p_Average_Energy[7], p_Average_Energy[8], p_Average_Energy[9], layout = (3, 3), size = [1920, 1080])
    ...
    Density_Plot = plot(p_Density[1], p_Density[2], p_Density[3], p_Density[4], p_Density[5], p_Density[6], p_Density[7], p_Density[8], p_Density[9], layout = (3, 3), size = [1920, 1080])
    ...
    Density_Histogram_Plot = plot(p_Histogram_Density[1], p_Histogram_Density[2], p_Histogram_Density[3], p_Histogram_Density[4], p_Histogram_Density[5], p_Histogram_Density[6], p_Histogram_Density[7], p_Histogram_Density[8], p_Histogram_Density[9], layout = (3, 3), size = [1920, 1080])
    ...
    Density_Average_Plot = plot(p_Average_Density[1], p_Average_Density[2], p_Average_Density[3], p_Average_Density[4], p_Average_Density[5], p_Average_Density[6], p_Average_Density[7], p_Average_Density[8], p_Average_Density[9], layout = (3, 3), size = [1920, 1080])
